...
out:
	while (__sync_fetch_and_add(&done, 0) < NTHREADS) {
		...
		//unintr_printf("tnp: %d\n", num_wait);
		unintr_printf("pre-spin\n");
		spin(5000);
		unintr_printf("post-spin\n");
		//unintr_printf("tnp: %d\n", num_wait);
...
/*
 * Parent thread creates N children threads. Children thread then wait on the
 * parent thread. Then parent kills roughly N/2 children threads via
 * thread_kill. Parent thread then calls thread_exit. The killed children thread
 * should not run any further, while the rest of the children should run to
 * completion. For each child thread, we should see one of "thread woken",
 * "parent gone", or "thread killed".
 */
void
test_wait_parent(void)
{
	...
	/* make sure some threads start waiting before we kill */
	ret = thread_yield(THREAD_ANY);
	/* we can't do this assert. if thread_yield is invoked after all child
	   threads are waiting, then thread_yield will return THREAD_NONE. */
	/* assert(thread_ret_ok(ret)); */

	/* kill some threads to test correct removal from wait queue */
	for (i = 0; i < NTHREADS / 2; i++) {
		int rand = random() % NTHREADS;
		ret = thread_kill(wait[rand]);

		if (ret == wait[rand]) {
			__sync_fetch_and_add(&done, 1);
			unintr_printf("%d: thread killed\n", wait[rand]);
		} else {
			assert(ret == THREAD_INVALID);
		}
